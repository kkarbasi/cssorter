...
class GaussianMixture():
    ...
    def initialisation(self, data):
        ...
        self.input = tf.compat.v1.placeholder(tf.float32, [None, self.DIMENSIONS])
        self.alpha = tf.compat.v1.placeholder_with_default(tf.cast(1.0, tf.float32), [])
        self.beta = tf.compat.v1.placeholder_with_default(tf.cast(1.0, tf.float32), [])
        ...
        initial_means = tf.compat.v1.placeholder_with_default(
                tf.cast(gmm.means_, tf.float32),
                shape=[self.COMPONENTS, self.DIMENSIONS]
             )


        initial_covariances = tf.compat.v1.placeholder_with_default(
        ...
        initial_weights = tf.compat.v1.placeholder_with_default(
    ...
    def computeLL(self, exp_log_shifted_sum, log_shift):
        ...
        log_likelihood = tf.reduce_sum(tf.math.log(exp_log_shifted_sum)) + tf.reduce_sum(log_shift)
    ...
    def fit(self, data):
        ...
        self.sess.run(tf.compat.v1.global_variables_initializer(), feed_dict={self.input: data[batch_idx]})
