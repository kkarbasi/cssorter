...
class ComplexSpikeSorter:
    ...
    def run(self):
        ...
        if delta >= self.signal_size:
            self._detect_spikes_gpu()
        else:
            self._detect_spikes_minibatch_gpu()
    ...
    def _detect_spikes_gpu(self):
        """
        Detect spikes using tensorflow implementation of online  gmm
        """
        from .GMM import GaussianMixture
        COMPONENTS = self.num_gmm_components 
        BATCH_SIZE = int(self.voltage.size)
        TRAINING_STEPS = 50
        TOLERANCE = 1e-4
        T0 = 500
        alpha_eta = 0.6
        ...
        gmm.fit(self.voltage_filtered.reshape(-1,1))
        ll, features = gmm.predict_proba(self.voltage_filtered.reshape(-1,1))
        labels = np.argmax(features, axis=0)
        spike_indices = np.where(labels == np.argmax(gmm.means.eval()))[0]
        peak_times,_ = scipy.signal.find_peaks(self.voltage_filtered[spike_indices])
    ...
    def _detect_spikes_gpu_from_range(self, prange):
        """
        Detect spikes using tensorflow implementation of online  gmm, only a range of signal
        """
        from .GMM import GaussianMixture
        COMPONENTS = self.num_gmm_components 
        voltage_signal = self.voltage_filtered[prange]
        signal_unfiltered = self.voltage[prange]
        BATCH_SIZE = int(voltage_signal.size)
        TRAINING_STEPS = 50
        TOLERANCE = 1e-4
        T0 = 500
        alpha_eta = 0.6
        ...
        gmm = GaussianMixture(COMPONENTS=COMPONENTS, BATCH_SIZE = BATCH_SIZE,
                TRAINING_STEPS=TRAINING_STEPS,
                TOLERANCE = TOLERANCE, verbose=2)
        gmm.fit(voltage_signal.reshape(-1,1))
        ll, features = gmm.predict_proba(voltage_signal.reshape(-1,1))
        labels = np.argmax(features, axis=0)
        spike_indices = np.where(labels == np.argmax(gmm.means.eval()))[0]
        peak_times,_ = scipy.signal.find_peaks(voltage_signal[spike_indices])
        spike_indices = spike_indices[peak_times]
        spike_peaks = np.array([np.argmin(signal_unfiltered[max(0, si - int(0.0005/self.dt)) :
            si + int(0.002/self.dt)]) for si in spike_indices])
    ...
    def _detect_spikes_minibatch_gpu(self):
        """
        ...
        """
        print('Using gpu minibatch spike detection, batch size = {}s'.format(self.minibatch_thresh))
        delta = int(self.minibatch_thresh/self.dt)
        self.spike_indices = np.array([], dtype='int64')
        for i in np.arange(0, self.voltage_filtered.size - int(10/self.dt), delta):
            curr_indices = self._detect_spikes_gpu_from_range(slice(i, i + delta)) 
